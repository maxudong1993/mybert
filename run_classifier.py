...
class MyProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        filename = data_dir + '/devset.json'
        return self._create_examples(filename, "devset")
    def get_test_examples(self, data_dir):
    ...
    def _create_examples(self, filename, set_type):
        ...
        with open(filename,'r') as f:
            ...
            if set_type == 'test':
                ...
                ids = my_dict.keys()
            for c_id in ids:
                cur_claim = my_dict[c_id]
                text_a =  tokenization.convert_to_unicode(cur_claim['claim'])
                if set_type != 'test':
                    label = tokenization.convert_to_unicode(cur_claim['label'])
                evidences = cur_claim['evidence']
                for i, evidence in enumerate(evidences):
                    tokens = evidence.split()
                    useless_length = len(tokens[0]) + len(tokens[1]) + 2
                    text_b = tokenization.convert_to_unicode(evidence[useless_length:])
